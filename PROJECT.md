...
#### 菜單與訂單系統修復
- ✅ **修復後台菜單資料同步問題**：
  - 移除 `newMenu.length >= 100` 的限制，確保所有菜單資料都能正確從 Firestore 同步。
  - 解決因同步問題導致的資料遺失風險。
- ✅ **修復客製化選項金額輸入**：
  - 修正後台客製化選項價格輸入框，現在可以正確輸入並顯示 `0` 元（例如免費選項）。
- ✅ **優化後台 Modal 佈局**：
  - 加寬訂單編輯與菜單編輯的 Modal 寬度 (600px -> 800px/90%)。
  - 優化客製化選項輸入框的排版，解決窄螢幕下「跑版」問題。
- ✅ **修復圖片顯示邏輯**：
  - 後台自動隱藏無效的 `/placeholder.jpg`，改為顯示預設圖示。
  - 確保前台與後台圖片顯示一致。

#### 前台體驗優化
- ✅ **優化菜單卡片顯示**：
  - 將菜單項目的名稱與價格改為**垂直排列**。
  - 解決名稱過長時與價格擠在同一行造成的斷行與排版混亂問題。

#### 系統架構調整
- ✅ **StorageService 功能擴充**：
  - 新增 `deleteMenuItem` 方法，支援從 Firestore 直接刪除菜單項目。
- ✅ **資料庫集合釐清**：
  - 確認系統主要使用 `menuItems` 集合，`menu` 集合為棄用狀態。
...
*此文件由 Antigravity AI 於 2025-12-01 更新*
*包含完整的專案流程、部署步驟、功能說明與疑難排解指南*
*最新更新：後台 Modal 跑版修復、客製化選項輸入修正、前台菜單垂直排版優化*
